...
describe("Clipboard interface", () => {
  ...
  it("uses style tags included in the paste", () => {
    let view = tempEditor({doc: doc(p())})
    ist(parseFromClipboard(view, "x", "<p><span class=b>bold</span> text</p><style>span.b { font-weight: bold }</style>",
                           false, view.state.doc.resolve(1)),
        doc(p(strong("bold"), " text")).slice(1, 10, true), eq)
  })
