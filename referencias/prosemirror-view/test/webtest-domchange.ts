...
describe("DOM change", () => {
  ...
  it("doesn't create steps in the middle of surrogate pairs when overwriting", () => {
    // 𝛾 (U+1D6FE) and 𝛽 (U+1D6FD) are astral chars that share the same high
    // surrogate (\uD835) and differ only in the low surrogate. Overwriting one
    // with the other produces a non-UTF-8 step content and resulting document.
    let steps: Step[] = [], view = tempEditor({
      doc: doc(p("𝛾<a>")),
      dispatchTransaction(tr) {
        steps = tr.steps
    ...
    findTextNode(view.dom, "𝛾")!.nodeValue = "𝛽"
    flush(view)
    ist(steps.length, 1)
    ist((steps[0] as any).slice.content.toString(), '<"𝛽">')
  })
})
