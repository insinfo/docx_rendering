...
describe("DOMSerializer", () => {
  ...
  it("supports {dom, contentDOM} DOMOutputSpec with element nodes", () => {
    let wrapperSchema = new Schema({
      nodes: {
        doc: {content: "wrapper+"},
        wrapper: {
          content: "text*",
          ...
          parseDOM: [{tag: "div.outer", contentElement: "p.inner"}]
        },
        text: {}
      }
    })
    let b = builders(wrapperSchema)
    let dom = document.createElement("div")
    dom.appendChild(DOMSerializer.fromSchema(wrapperSchema).serializeFragment(b.doc(b.wrapper("hello")).content, {document}))
    ist(dom.innerHTML, '<div class="outer"><p class="inner">hello</p></div>')
  })

  it("supports raw HTMLElement DOMOutputSpec", () => {
    ...
    let b = builders(leafSchema)
    let dom = document.createElement("div")
    dom.appendChild(DOMSerializer.fromSchema(leafSchema).serializeFragment(b.doc(b.leaf()).content, {document}))
    ist(dom.innerHTML, '<hr>')
  })
